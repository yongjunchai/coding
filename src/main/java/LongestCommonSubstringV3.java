...
public class LongestCommonSubstringV3 {
    ...
    /**
     * subProblems[i][j] = :
     *      case 1:  if left[i - 1] == right[j - 1] and there is no dup
     *          subProblems[i][j] = subProblems[i - 1][j - 1] + 1
     *      case 2: if left[i - 1] == right[j - 1] and there is dup
     *          subProblems[i][j] = 1
     *      case 3: if left[i - 1] != right[j - 1]
     *          subProblems[i][j] = 0
     *
        ...
        for (int i = 1; i <= left.length; ++ i) {
            for (int j = 1; j <= right.length; ++ j) {
                if (left[i - 1] == right[j - 1]) {
                    //case 2
                    if (subProblems[i - 1][j -1] > 0 && left[i - 1] == left[i -2]) {
                        subProblems[i][j] = 1;
                    }
                    else {
                    //case 1
